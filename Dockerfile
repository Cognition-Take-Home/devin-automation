# Container image for the dependency usage-optimization app.
#
...
#             devin-automation shortlist
FROM python:3.12-slim

# git: read the target repo / commit state.  ripgrep: measure dependency usage.
# gh: optional, powers the PR "rework" metric.
RUN apt-get update \
    && apt-get install -y --no-install-recommends git ripgrep gh ca-certificates \
